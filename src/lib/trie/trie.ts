...
function searchWordByTreeTraverse(stack: StackItem[], wordsArray: string[], separator: string, length: number): Observable<string[]> {
...
export class Trie {
    ...
    getRandomFullString(length: number, separator: string = ''): string {
        ...
        combineLatest([
            searchWordByTreeTraverse(stack, [], separator, length),
            searchWordByTreeTraverse(stack, [], separator, length),
            searchWordByTreeTraverse(stack, [], separator, length),
        ]).pipe(
            mergeMap(fn => fn, 1000),
        ...
        return shuffleArray(wordsArray).join(separator).substring(0, length)
    }
